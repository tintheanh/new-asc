...
export const indexStudent = functions.firestore
...
export const unIndexStudent = functions.firestore
  .document('students/{studentID}')
  .onDelete((snap, _) => {
    const objId = snap.id;
...
export const updateIndexStudent = functions.firestore
  .document('students/{studentID}')
  .onUpdate((change, _) => {
...
export const deleteUser = functions.https.onCall((data, context) => {
  ...
  if (context.auth) {
    ...
    throw new functions.https.HttpsError(
      'permission-denied',
      'The function must be called with authentication.'
    );
  }
});

export const updateUserEmail = functions.https.onCall((data, context) => {
  ...
  if (context.auth) {
    return admin
      .auth()
      .updateUser(uid, {
        email: `${newEmail}@asc.com`
      })
      .then(() => {
        console.log('Successfully update email user');
      });
  } else {
    throw new functions.https.HttpsError(
      'permission-denied',
...
export const checkPastAppointments = functions.pubsub
  .topic('check-my-appointments')
