...
class TutorScheduleReportScreen extends React.Component<any, TutorScheduleReportStates> {
						...
						{chunk.map((tutor: any, g: number) => {
							return (
								<div key={g}>
									...
									{Object.keys(tutor.work_schedule).map((key: string, j: number) => {
										if (tutor.work_schedule[key].length) {
											return (
												<div key={j}>
													<div className={styles.dayReport}>
														<h4>{workDays[Number(key)]}</h4>
														{tutor.work_schedule[key].map((shift: any, k: number) => {
															if (k % 2 === 0)
																return (
	...
	processData = () => {
		...
		const results = chunked.map((ch: any) => {
			...
			const groups = Object.keys(group_to_values).map((key: any) => {
				const tutor = tutorsWithObjWorkSchedule.filter((tt: any) => tt.uid === key)[0];
