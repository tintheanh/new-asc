...
class SubjectByTutorReportScreen extends React.Component<any, any> {
					...
					<section className={`sheet padding-15mm ${styles.page}`}>
						...
						<div className={styles.subjectContainer}>
							{ch.map((subject: any, j: number) => {
								return (
									<div key={j}>
										<h4 className={styles.subjectName}>{`${subject.label} - ${subject.full}`}</h4>
										{subject.tutors.map((tutor: any, k: number) => {
											return (
												<div
													className={`${styles.tutor} ${k % 2 === 0
														? styles.even
														: styles.odd}`}
													key={k}
												>
	...
	processData = () => {
		const subjectWithTutor = this.state.data.map((subject: any) => {
			return subject.tutors.map((tutor: any) => ({ name: tutor.name, uid: tutor.uid, subject: subject.id }));
		});
		const flatted = subjectWithTutor.flat(Infinity);
		const chunked = chunk(flatted, 12);
		...
		const results = chunked.map((ch: any) => {
			...
			const groups = Object.keys(group_to_values).map((key: any) => {
				const subject: any = this.state.data.filter((sj: any) => sj.id === key)[0];
				return { ...subject, tutors: group_to_values[key] };
			});
			return groups;
		});
		return results;
