...
import {
  EditTutorSubjectTableProps,
  EditTutorSubjectTableStates
} from './props';
...
class EditTutorSubjectTable extends React.Component<
  EditTutorSubjectTableProps,
  EditTutorSubjectTableStates
> {
  ...
  handleOnKey = (behavior: 'up' | 'down') => (e: React.KeyboardEvent) => {
    if (e.keyCode === 17)
      this.setState({
        toggleSelectCtrl: behavior === 'down' ? true : false,
        toggleSelectShift: false
      });
    if (e.keyCode === 16)
      this.setState({
        toggleSelectShift: behavior === 'down' ? true : false,
        toggleSelectCtrl: false
      });
  };
  ...
  selectSubjects = (key: string, indexKey: string, subject: Subject) => () => {
    ...
    if (toggleSelectCtrl) {
      ...
      if (
        ...
        !contains(selectedSubjects, subjects[index], 'id')
      ) {
      ...
      if (
        ...
        if (index > indexes[indexes.length - 1]) {
          ...
          for (let i = indexes[0]; i <= indexes[indexes.length - 1]; i += 1) {
            if (!contains(selectedSubjects, subjects[i], 'id'))
        ...
        if (indexes[0] < indexes[indexes.length - 1]) {
          ...
          for (
            let i = newIndexes[0];
            i <= newIndexes[newIndexes.length - 1];
            i += 1
          ) {
            selectedSubjects.push(subjects[i]);
          }
      ...
      if (
        ...
        this.state[key][0].id !== subjects[index].id
      ) {
      ...
      switch (type) {
        case 'add': {
          ...
          for (const subject of subjects) {
            if (!contains(existingSubjects, subject, 'id'))
        ...
        case 'remove': {
          const newSubjects = selected.subjects.filter(
            (sj: Subject) => !contains(subjects, sj, 'id')
          );
  ...
  _preprocessSubjects = () => {
    return this.props.subjects.filter(
      sj => !contains(this.props.selected.subjects, sj, 'id')
    );
  };
      ...
      return (
        <div className={styles.container}>
          <h3 className={styles.title}>
            {this.props.selected.first_name} {this.props.selected.last_name}'s
            subjects
          </h3>
          <ReactTable
            ...
            pageSize={
              this.props.selected.subjects.length < 5
                ? 5
                : this.props.selected.subjects.length
            }
            columns={columns}
            ...
            getTrProps={(_: any, rowInfo: any) => {
              if (rowInfo && rowInfo.row) {
                ...
                return {
                  ...
                  style: {
                    background: contains(removedSubjects, subject, 'id')
                      ? 'rgba(22, 96, 27, 0.7)'
                      : 'none',
                    color: contains(removedSubjects, subject, 'id')
                      ? 'white'
                      : 'black'
                  }
                };
          ...
          <ReactTable
            ...
            pageSize={
              this._preprocessSubjects().length < 5
                ? 5
                : this._preprocessSubjects().length
            }
            columns={columns}
            showPagination={false}
            sortable={false}
            ...
            getTrProps={(_: any, rowInfo: any) => {
              if (rowInfo && rowInfo.row) {
                ...
                return {
                  ...
                  style: {
                    background: contains(addedSubjects, subject, 'id')
                      ? 'rgba(22, 96, 27, 0.7)'
                      : 'none',
                    color: contains(addedSubjects, subject, 'id')
                      ? 'white'
                      : 'black'
                  }
                };
...
export default connect(
  mapStateToProps,
  { selectAndUpdateTutor, fetchAllSubjects, updateTutor }
)(EditTutorSubjectTable);
