...
class EditOffTimes extends React.Component<any, any> {
  ...
  setTimeFrom = (time: any) =>
    this.setState({ fromTime: time.format('hh:mm A') });
  ...
  addOffTime = () => {
    if (this.state.from && this.state.to) {
      ...
      const timeSet = {
        from: Number(
          moment(
            `${from.toLocaleDateString('en-US')} ${this.state.fromTime}`,
            'M/D/YYYY hh:mm A'
          ).format('X')
        ),
        to: Number(
          moment(
            `${to.toLocaleDateString('en-US')} ${this.state.toTime}`,
            'M/D/YYYY hh:mm A'
          ).format('X')
        )
      };
      if (timeSet.from < timeSet.to) {
        ...
        const sorted = processedTime.sort((a, b) =>
  ...
  deleteOffTime = () => {
    ...
    const newOffTimes = tutor.off_time.filter(
      (time: any) => time.from !== this.state.timeSelected.from
    );
  ...
  handleUpdate = () => {
    if (this.props.selected) {
      ...
      this.props
        .updateTutor(selected, data, false)
        .then(() => close())
  ...
  render() {
    ...
    return (
      <div>
        <h3>
          {this.props.selected.first_name} {this.props.selected.last_name}'s Off
          Times
        </h3>
        <div className={styles.container}>
          <div>
            <ReactTable
              ...
              pageSize={
                this.processData().length < 9 ? 9 : this.processData().length
              }
                ...
                if (rowInfo && rowInfo.row) {
                  ...
                  if (this.state.timeSelected) {
                    return {
                      ...
                      style: {
                        background:
                          this.state.timeSelected.from === time.from
                            ? 'rgba(22, 96, 27, 0.7)'
                            : 'none',
                        color:
                          this.state.timeSelected.from === time.from
          ...
          <div>
            ...
            <div className={styles.btnWrapper}>
              ...
              />
              <Button
                disabled={this.state.timeSelected.from === 0}
                label="Delete"
                onClick={this.deleteOffTime}
              />
            </div>
            <Button
...
export default connect(
  mapStateToProps,
  { selectAndUpdateTutor, updateTutor }
)(EditOffTimes);
